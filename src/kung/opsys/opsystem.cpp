	...
	for (auto op : all) {
		switch (op->type) {
			case OpType::OP_TYPE_REQUESTER:
				op->request(op); // Updates the whole call graph
				break;
			case OpType::OP_TYPE_SENDER:
				op->update(op); // A Sender will call other nodes itself, which then call other nodes etc. like snowball effect
				break;
	...
	for (auto op : changed_ops) {
		for (int i=0; i<op->number_of_outputs; i++) {
			...
			for (auto il : *ol->inputlinks) {
				il->owner->on_link_update(il->owner, il->local_id);
			}
		}
	}
	changed_ops.clear();
...
void OpSystem::regenerateCallGraphs() {
	...
	for (auto op : all) {
		if ( IS_EXPRESSION(op) || IS_PREFAB_INPUT(op) || IS_PREFAB_OUTPUT(op) ) {
	...
	for (auto op : all) {
		if ( ( IS_SWITCH(op) || IS_KEYBOARD(op) ) && op->cache_ingoing_connections==0 ) {
	...
	for (auto op : requesters) {
		op_generate_callgraph(op, &op->update_ops_this_order);
...
static void op_create_link(Op *from, int output_id, Op *to, int input_id) {
	
	if (from == NULL) {
		log("op_system> from is NULL\n");
		return;
	}
	if (to == NULL) {
		log("op_system> to is NULL\n");
		return;
	}

	LinkOutput *output_list = op_getOutput(from, output_id);
	...
	Link *input_link = op_getInput(to, input_id);
	if ( ! input_link)
...
Op *OpSystem::op_from_string(char *line) {
	...
	char buf[1024];
	int ret = sprintf(buf, "%63s pos %%f %%f size %%f %%f selected %%d perm %%d name %%63s %%[^\n]", firstword);
	...
	if ( ! op) {
		log("could not create op via found->create_op() method, line:%s", line);
		return NULL;
	}
...
void OpSystem::fixNameCollisions() {
	...
	for (auto op : all) {
		...
		if (id2op.find(key) != id2op.end()) {
			...
			char newname[64];
			sprintf(newname, "%s#%x", op->name, op);
			log("name collision for %s, renaming to %s\n", op->name, newname);
			strcpy(op->name, newname);
			key = op->name;
		}
		id2op[key] = op;
	}
...
void OpSystem::saveFile(char *filename) {
	...
	if ( ! f) {
		log("Can't open file to write: \"%s\" Error: %s\n", filename, PHYSFS_getLastError());
		return;
	}
	fixNameCollisions();
	for (auto op : all) {
...
void OpSystem::updateExecIDs() {
	...
	expressions.clear();
	for (auto op : all) {
		if ((int)op->render == (int)op_expression_render) {
...
void OpSystem::updatePrefabInputIDs() {
	...
	prefabinputs.clear();
	for (auto op : all) {
		if ((int)op->render == (int)op_prefab_input_render) {
			//log("Found expression: %s\n", op->name);
...
void OpSystem::updatePrefabOutputIDs() {
	...
	prefaboutputs.clear();
	for (auto op : all) {
		if ((int)op->render == (int)op_prefab_output_render) {
			//log("Found expression: %s\n", op->name);
...
void OpSystem::updateRenderers() {
	...
	renderers.clear();
	for (auto op : all) {
		if ((int)op->render == (int)op_renderer_render) {
			//log("Found expression: %s\n", op->name);
...
void render_legacy(Node *node) {
	for (auto op : node->opsystem.renderablesLegacy) {
		...
		if (IS_GL_LEGACY((Op *)op)) {
			op_gl_legacy_render_legacy_code((OpGLLegacy *) op);
		}
		if (IS_XMODEL((Op *)op)) {
			op_xmodel_render_legacy_code((OpXModel *) op);
		}
	}
...
void OpSystem::updateRenderablesLegacy() {
	...
	for (auto op : all) {
		if (IS_GL_LEGACY((Op *)op) || IS_XMODEL((Op *)op)) {
			//log("Found expression: %s\n", op->name);
