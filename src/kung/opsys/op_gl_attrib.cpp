...
void METHOD(init)(CLASSNAME *op) {
	...
	op_init_link_output((Op *)op, 0, "AttLoc", OP_TYPE_GLATTRIB );
...
void METHOD(render)(CLASSNAME *op) {
	...
	ImGui::InputText("attrib", op->attrib, sizeof op->attrib);
...
void METHOD(update)(CLASSNAME *op) {
	...
	if (op->cache_ingoing_connections > 0) {
		op->default_link_outputs[0].opengl_id = glGetAttribLocation(op->default_link_inputs[0].opengl_id, op->attrib);
