...
int js_init() {
	...
	duk_eval_string(ctx, "function safeeval(global, code) { try { eval.bind(global)(code); } catch(e) { log(e.stack); }  }");
	duk_pop(ctx);
	...
	duk_eval_string(ctx, "handle_input = function(code) { try { ret = eval(code); log(ret) } catch (e) {log(\"error: \" + e + \"\\n\")} return \"not needed\"; }");
	duk_pop(ctx);
