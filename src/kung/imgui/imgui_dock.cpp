...
namespace ImGui
{
	...
	Dock::~Dock() { MemFree(label); }


	ImVec2 Dock::getMinSize() const
	{
	...
	bool Dock::isHorizontal() const { return children[0]->pos.x < children[1]->pos.x; }


	void Dock::setParent(Dock* dock)
	{
		parent = dock;
	...
	bool Dock::isContainer() const { return children[0] != nullptr; }


	void Dock::setChildrenPosSize(const ImVec2& _pos, const ImVec2& _size)
	{
	...
	void Dock::setPosSize(const ImVec2& _pos, const ImVec2& _size)
	{
		size = _size;
		pos = _pos;
...
Dock& DockContext::getDock(const char* label, bool opened)
{
...
Dock* DockContext::getDockByIndex(int idx) { return idx < 0 ? NULL : m_docks[idx]; }
