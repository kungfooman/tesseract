...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (pressed && !(flags & ImGuiTreeNodeFlags_Leaf))
    {
		...
		// kung foo man: so we can check if a tree node was only toggled, so ImGui::IsItemClicked() should not selected the node
		treenode_toggled = toggled;
		//log("treenode_toggled: %d framecounter=%d\n", treenode_toggled, framecounter);



        if (toggled)
        {
			//if (node_current_rendering) {
			//	//node_current_rendering->selected = !node_current_rendering->selected;
			//}
            is_open = !is_open;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id)
    {
		...
		// kung foo man: ESC only closes menu for me, it would loose last-typed data everytime
