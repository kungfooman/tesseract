    ...
    float fov, int ortho, int radius)
{
	...
	if (first) {
		...
		//
		//	// load the system
		cursys = sys_ortho; // otherwise op_from_string uses the wrong system to check if a op is already added
		OpSystem::SetCurrent(sys_ortho);
	...
	if ( ! ortho) {
		craft_matrix_persp->request(craft_matrix_persp);
		matcpy(matrix, craft_matrix_persp->matrix);
	} else {
		ortho_result->request(ortho_result);
